...
Encoder::Encoder(int pin, int ticksPerRotation) : filter(100, 100, 0.08) {
    ...
    _rotationsPerTick = 1/float(ticksPerRotation); // Calculate this now for efficiency later
...
void Encoder::update() {
    ...
    if (lastTickTime != lastUpdateTime) {
        speed = tick_count * (tickConversion / float(lastTickTime - lastUpdateTime));
    } else {
        speed = 0;
    }
...
float Encoder::getPos() {
    return totTicks * _rotationsPerTick;
}
...
*/
void Encoder::isr(unsigned instance_num) {
    instances[instance_num]->tick();
}
