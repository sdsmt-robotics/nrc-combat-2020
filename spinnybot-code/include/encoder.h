...
class Encoder {
   ...
   private:
    ...
    volatile unsigned long lastUpdateTime;  // time in microseconds of the tick preceding the last estimate

    // Conversion from (ticks / us) -> (rot / min)
    int _ticksPerRotation = 360;
    float _rotationsPerTick = 1 / float(_ticksPerRotation);
    unsigned long tickConversion = 1000000ul * 60 * _rotationsPerTick;  // (us*rotations)/(min*tick)
