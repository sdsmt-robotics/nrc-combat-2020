...
int AS5134::readCounter() {
    int numTurns = getData(RD_MT_COUNTER) >> 7;
...
* @return the data received if in read mode.
*/
uint64_t AS5134::transmitEx(uint8_t command, bool sendMode, uint64_t data, uint8_t length) {
 //NOTE: Delays have been removed from this. Max clock frequency of the encoder is 6MHZ. 
    // Might need to slow this down if there are problems.
    ...
    // digitalWrite(clkPin,LOW);
    // delayMicroseconds(2);
    // digitalWrite(dioPin, ((command >> 4) & 1));
    // delayMicroseconds(1);
    // digitalWrite(clkPin,HIGH);
    // delayMicroseconds(7);
    // digitalWrite(clkPin,LOW);
    // digitalWrite(dioPin, ((command >> 3) & 1));
    // delayMicroseconds(2);
    // digitalWrite(clkPin,HIGH);
    // delayMicroseconds(1);
    // digitalWrite(dioPin, ((command >> 2) & 1));
    // delayMicroseconds(1);
    // digitalWrite(clkPin,LOW);
    // delayMicroseconds(1);
    // digitalWrite(dioPin, ((command >> 1) & 1));
    // delayMicroseconds(1);
    // digitalWrite(clkPin,HIGH);
    // delayMicroseconds(7);
    // digitalWrite(clkPin,LOW);
    // delayMicroseconds(1);
    // digitalWrite(dioPin, (command & 1));
    // delayMicroseconds(1);
    // digitalWrite(clkPin,HIGH);
    // delayMicroseconds(7);
    digitalWrite(clkPin,LOW);
    ...
    for(int bitNum=length-1; bitNum >= 0; bitNum--) {
        //tick the clock LOW
        for(int i=0;i<4;i++)  // Extended mode messages take 4 tickss
        {
            //send the bit if in send mode
            if (sendMode && i == 0) {
                digitalWrite(dioPin, ((data >> bitNum) & 1));
